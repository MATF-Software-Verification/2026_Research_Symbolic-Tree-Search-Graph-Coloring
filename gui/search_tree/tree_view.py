...
class SearchTreeWidget(QGraphicsView):
    ...
    def show_coloring_info(self, coloring: List[int], is_valid: bool, conflict=None):
        ...
        """
        # Get mapping from main_window if available
        node_id_mapping = None
        if self.main_window and hasattr(self.main_window, '_node_id_mapping'):
            node_id_mapping = self.main_window._node_id_mapping

        self._info_panel.show_coloring(coloring, is_valid, conflict, node_id_mapping)
        self._position_info_panel()
