...
class NodeColorRow(QWidget):
    ...
    def set_node_id(self, node_id: int):
        """Update the displayed node ID."""
        self.node_id = node_id
        self.node_label.setText(f"Node {node_id}:")
...
class ColoringInfoPanel(QFrame):
    ...
    def __init__(self, parent=None):
        ...
        # Store coloring data
        self._coloring: Optional[List[int]] = None
        self._node_id_mapping: Optional[Dict[int, int]] = None
        self._reverse_mapping: Optional[Dict[int, int]] = None
    ...
    def _set_conflict(self, coloring: List[int], conflict):
        ...
        # Build readable list using proper index mapping
        parts = []
        for u, v in conflicts:
            # Convert original node IDs to coloring indices
            if self._node_id_mapping:
                u_idx = self._node_id_mapping.get(u)
                v_idx = self._node_id_mapping.get(v)
                if u_idx is None or v_idx is None:
                    continue
                if u_idx >= len(coloring) or v_idx >= len(coloring):
                    continue
                color_value = coloring[u_idx]
            else:
                # No mapping - assume IDs are consecutive
                if u >= len(coloring) or v >= len(coloring):
                    continue
                color_value = coloring[u]
            
            parts.append(f"({u}-{v}) same color {color_value}")
    ...
    def _update_node_rows(self, coloring: List[int]):
        ...
        # Build reverse mapping if we have node_id_mapping
        if self._node_id_mapping:
            self._reverse_mapping = {v: k for k, v in self._node_id_mapping.items()}
        else:
            self._reverse_mapping = None

        # Add new rows
        for idx, color_value in enumerate(coloring):
            # Get original node ID if mapping exists
            if self._reverse_mapping:
                original_node_id = self._reverse_mapping.get(idx, idx)
            else:
                original_node_id = idx
            row = NodeColorRow(original_node_id)
    ...
    def show_coloring(self, coloring: List[int], is_valid: bool, conflict=None, node_id_mapping: Dict[int, int] = None):
        """
    ...
    def show_partial_coloring(self, partial_coloring: List[int],  node_id_mapping: Optional[Dict[int, int]] = None):
        """
        Display a partial coloring (for inner tree nodes) in the panel.
        """
        self._coloring = partial_coloring
        self._node_id_mapping = node_id_mapping
